...
export const Icreate_route_transfers__postDTOSchema = z.object({
  currency: z.enum(["EGP"]),
...
export const Ilist_route_transfers__getQuerySchema = z.object({
...
export const Iupdate_route_transfers__id__putDTOSchema = z.object({
  currency: z.enum(["EGP"]),
  amount: z.union([z.number(), z.string().regex(/^(?!^[-+.]*$)[+-]?0*\d*\.?\d*$/)]),
  notes: z.union([z.string(), z.null()]).optional(),
  financial_flow_id: z.union([z.string().uuid(), z.null()]).optional(),
